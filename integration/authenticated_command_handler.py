...
CONFIG = {}
RUNNING = True
COMMAND_HISTORY = defaultdict(list)  # Track commands for replay protection


def load_config(config_path: str = "/etc/authenticated-repeater/config.yaml") -> Dict:
...
def check_replay_protection(command_data: Dict, config: Dict) -> bool:
    ...
    if current_time - timestamp > window:
        logging.warning(f"Command timestamp too old: {timestamp} (current: {current_time})")
        return False
    
    # Check if timestamp is in the future (clock skew)
    if timestamp > current_time + 60:  # Allow 60 second clock skew
        logging.warning(f"Command timestamp in future: {timestamp} (current: {current_time})")
        return False
    ...
    if len(recent_commands) > max_per_minute:
        logging.warning(f"Rate limit exceeded for {callsign}: {len(recent_commands)} commands in last minute")
        return False
...
def execute_svxlink_command_config(command: str, config: Dict) -> Tuple[bool, str]:
    """
    ...
    """
    config_file = config.get('svxlink_config', '/etc/svxlink/svxlink.conf')
...
def process_command(command_frame: bytes, signature_frame: bytes, authorized_keys: Dict[str, bytes], config: Dict) -> Tuple[bool, str]:
    """
