...
class M17AttackVectors:
    ...
    def generate_attack_vectors():
        ...
        vectors.append(
            (
                "multiple_sync",
                struct.pack(">H", SYNC_LSF) + b"\x00" * 20 + struct.pack(">H", SYNC_LSF) + b"\x00" * 20,
                True,
            )
        )
        ...
        vectors.append(
            ("mixed_frames", M17AttackVectors.create_lsf_frame() + M17AttackVectors.create_packet_frame(), True)
        )
        ...
        vectors.append(
            (
                "special_bytes",
                struct.pack(">H", SYNC_LSF) + bytes([0x00, 0xFF, 0x80, 0x7F, 0x01, 0xFE]) + b"\x00" * 40,
                True,
            )
        )
        ...
        vectors.append(
            ("sync_like_payload", struct.pack(">H", SYNC_LSF) + struct.pack(">H", SYNC_PACKET) + b"\x00" * 44, True)
        )
