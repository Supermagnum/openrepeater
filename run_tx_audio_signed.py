...
def main():
    ...
    start_button.setStyleSheet(
        "QPushButton { background-color: #4CAF50; color: white; font-weight: bold; padding: 10px; min-width: 150px; }"
    )
    stop_button = Qt.QPushButton("Stop")
    stop_button.setStyleSheet(
        "QPushButton { background-color: #f44336; color: white; font-weight: bold; padding: 10px; min-width: 150px; }"
    )
        ...
        if not tb.flowgraph_started.is_set():
            ...
            if not tb.flowgraph_started.is_set():
                ...
                tb.blocks_wavfile_sink_0 = blocks.wavfile_sink(
                    tb.output_audio_file, 1, tb.samp_rate, blocks.FORMAT_WAV, blocks.FORMAT_PCM_16, False
                )
