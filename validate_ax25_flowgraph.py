...
def validate_frame_positions(frames, total_samples, voice_end_sample, sample_rate=48000):
    ...
    if len(validation_frames) >= 2:
        if validation_frames[0]['start_sample'] >= validation_frames[1]['start_sample']:
            details['errors'].append(
                "Frame ordering incorrect: frame 1 should come before frame 2"
            )
    ...
    success = (
        len(validation_frames) >= 2 and
        frames_in_audio == 0 and  # Redundant but explicit
        all(f['start_sample'] >= frame_end_threshold for f in validation_frames) and
        all(f['start_sample'] >= audio_portion_end for f in validation_frames)
    )
...
def main():
    ...
    if len(details['frames']) > 0:
        ...
        for frame_info in details['frames']:
            ...
            print(f"     After voice end: {frame_info['after_voice_end']} (voice ends at {details['voice_end_sample']})")
    ...
    if success:
        ...
        print("=" * 70)
        print("\nSummary:")
        print(f"  ✓ Audio portion (0 to {details['voice_end_sample']} samples, {details['voice_end_percent']:.1f}% of file):")
        ...
        if len(details['frames']) > 0:
            ...
            if frames_in_audio > 0:
                audio_frame_nums = [f['frame_number'] for f in details['frames'] if f.get('in_audio_portion', not f['after_voice_end'])]
