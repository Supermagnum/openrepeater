...
// KISS TNC State
typedef enum { KISS_STATE_IDLE, KISS_STATE_FEND, KISS_STATE_DATA, KISS_STATE_ESCAPE } kiss_state_t;
...
// Utility Functions
int kiss_escape_data(const uint8_t* input, uint16_t input_len, uint8_t* output,
                     uint16_t* output_len);
int kiss_unescape_data(const uint8_t* input, uint16_t input_len, uint8_t* output,
                       uint16_t* output_len);
