...
# Title: Full-Duplex Adaptive Modulation System
# Author: gr-packet-protocols
# Description: Complete full-duplex adaptive system with TX/RX, link quality monitoring,
...
class adaptive_full_duplex_flowgraph(gr.top_block):
    """
    Complete full-duplex adaptive modulation system with:
    - TX Path: Source -> Encoder -> Link Quality Monitor -> Adaptive Modulator -> Sink
    - RX Path: Source -> Demodulator -> Decoder -> Link Quality Monitor
    ...
    def __init__(self):
        gr.top_block.__init__(self, "Full-Duplex Adaptive System", catch_exceptions=True)
        ...
        # TX Output Sink (replace with USRP sink for SDR)
        self.tx_sink = blocks.file_sink(
        ...
        # For now, we'll use the one from adaptive modulator
        ...
        # ========================================================================
        # KISS TNC (for negotiation frames)
        # ========================================================================
        
        # KISS TNC - handles negotiation frames
        ...
        self.connect(self.tx_adaptive_mod, self.tx_sink)
        
        # RX Path
        self.connect(self.rx_source, self.rx_demod_2fsk)
        self.connect(self.rx_source, self.rx_snr_estimator)  # Parallel connection for SNR
        ...
        # Set callback for sending KISS frames (for negotiation)
        def send_kiss_frame_callback(command, data):
            """Callback to send KISS frames via TNC"""
            # In a real system, this would send frames over the radio
            # For now, we'll just log it
            print(f"Would send KISS frame: command=0x{command:02x}, length={len(data)}")
            # self.kiss_tnc.send_kiss_frame(command, 0, data, len(data))
        
        # Note: This requires the modulation_negotiation block to have set_kiss_frame_sender method
        # If not available, negotiation will need to be handled differently
        ...
        print("Full-duplex adaptive system initialized")
    ...
    def handle_snr_update(self, msg):
        ...
        try:
            ...
            # Update RX quality monitor
            self.rx_quality_monitor.update_snr(snr_db)
            # Also update TX quality monitor (assuming similar conditions)
...
def main(top_block_cls=adaptive_full_duplex_flowgraph, options=None):
    """Run the flowgraph"""
    tb = top_block_cls()
