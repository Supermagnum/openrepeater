...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    // Need at least key (16 bytes minimum)
    if (size < 16 || size > 1024) {
        return 0; // Reject invalid sizes
    }

    // Determine key size: 16, 24, or 32 bytes
    size_t key_size = 16;
    if (size >= 24 && (data[0] % 3 == 0)) {
        key_size = 24;
    }
    ...
    try {
        ...
        if (size > key_size + 16 + 1) {
            input_size = std::min(size - key_size - 16 - 1, static_cast<size_t>(2048));
            input_size = ((input_size + 15) / 16) * 16; // Pad to block size
        }

        unsigned char* input = new unsigned char[input_size];
        unsigned char* output = new unsigned char[input_size];
        
        // Fill input with remaining fuzzer data
        if (size > key_size + 16 + 1) {
        ...
        (void)block->work(static_cast<int>(input_size), inputs, outputs);
