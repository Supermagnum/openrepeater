...
def main():
    ...
    try:
        process = subprocess.Popen(
            [sys.executable, "tx_audio_signed.py", "--input", INPUT_AUDIO, "--output", OUTPUT_AUDIO], cwd=EXAMPLES_DIR
        )
    ...
    os.chdir(SCRIPT_DIR)
    result = subprocess.run([sys.executable, "validate_ax25_flowgraph.py", OUTPUT_AUDIO], capture_output=False)
